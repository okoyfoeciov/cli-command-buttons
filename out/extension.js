        ...
        try {
            ...
            if (selectedFolder) {
                if (selectedFolder.path === "CREATE_NEW") {
                    ...
                    const projectToDelete = await vscode.window.showQuickPick(subfolders.map((folder) => ({
                        label: `$(folder) ${folder.label}`,
                        description: folder.path,
                        path: folder.path,
                    })), {
                    ...
                    if (projectToDelete) {
                        try {
                            ...
                            vscode.window.showInformationMessage(`Deleted project: ${path.basename(projectToDelete.path)}`);
                        }
                        catch (error) {
...
async function getOrCreateTerminal(commandName) {
    const config = vscode.workspace.getConfiguration("cliCommandButtons");
    const terminalBehavior = config.get("terminalBehavior") ||
        "reuse";
